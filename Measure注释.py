...
def LayerStatistics(margin, marinRGB):
    ...
    line_ids = []
    #找属于线的bin
    for i, value in enumerate(candidate):  #遍历所有candidate,如果其值不为0,且不是已找到直线,认为是发现新的直线,记录区间中心位置,如果是的话,用于更新直线位置,如果value<0,
...
def GetMargin(gray):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            cv2.line(map, (x1, y1), (x2, y2), 128, 3)
